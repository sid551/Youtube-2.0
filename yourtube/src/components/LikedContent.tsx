...
export default function LikedVideosContent() {
  ...
  const loadLikedVideos = async () => {
    ...
    try {
      ...
      const validLiked = Array.isArray(likedData.data)
        ? likedData.data.filter((item: any) => item && item.videoid && item.videoid._id)
        : [];
      setLikedVideos(validLiked);
    } catch (error) {
            ...
            <div key={item._id} className="flex flex-col xs:flex-row gap-3 sm:gap-4 group bg-white dark:bg-zinc-900 xs:bg-transparent p-2.5 xs:p-0 rounded-xl">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem
                    onClick={() => handleUnlikeVideo(v._id, item._id)}
                  >
